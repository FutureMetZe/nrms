...
/*	@Autowired
	...
	public String toIndex(HttpSession session){
		...
		if(user == null){
			...
			return "redirect:/TaskPublishController/taskList";
		}
	}
	...
	@ResponseBody
	public ExecResult doLogin(String loginName , String pwd ,  HttpSession session){
...
//		}
		...
		Users user = this.usersService.userLogin(loginName);
		if(user == null){
			result.setSuccess(false);
			result.setMsg("帐号不存在");
			return result;
		...
		if(!user.getHashedPassword().equals(pwd)){
			result.setSuccess(false);
			result.setMsg("密码错误");
			return result;
		}
	...
	public static void main(String[] args){
		String pwdMD5 = SecurityUtil.md5("123456");
